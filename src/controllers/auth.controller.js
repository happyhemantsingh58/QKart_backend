...
const register = catchAsync(async (req, res) => {
  ...
  res.status(httpStatus.CREATED).send({ user: users, tokens: tokens });
});
...
const login = catchAsync(async (req, res) => {
  ...
  const users = await authService.loginUserWithEmailAndPassword(
    email,
    password
  );
  const tokens = await tokenService.generateAuthTokens(users);
  res.status(httpStatus.OK).send({ user: users, tokens: tokens });
});
